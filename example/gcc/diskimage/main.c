...
int usage() {
  fc_term_puts("DISKIMAGE /r <device> <filename>\n\n");
  fc_term_puts("DISKIMAGE /w <filename> <device>\n\n");
  return 1;
}
...
int create_image(struct DeviceServiceRoutine* source_dsr, char* source, struct DeviceServiceRoutine* dest_dsr, char* dest) {
  ...
  unsigned int iocode = fc_path_to_iocode(source);
  int ferr = fc_lvl2_sector_read(source_dsr->crubase, iocode, 0, sector_buf);
  if (ferr) {
    fc_term_puts("error reading source disk\n");
    return 1;
  }

  char volume[11];
  fc_str_ncopy(volume, sector_buf, 10);
  fc_term_puts("volume: ");
  fc_term_puts(volume);

  int total_sectors = *((int*)&(sector_buf[10]));
  fc_term_puts("\ntotal sectors: ");
  fc_term_puts(fc_str_from_uint(total_sectors));
  fc_term_puts("\n");
  ...
  if (ferr) {
    fc_term_puts("error opening destination file: ");
    fc_term_puts(dest);
    fc_term_puts("\n");
    return 1;
  }
  ...
  for(int sno = 0; sno < total_sectors; sno++) {
    ...
    if (ferr) {
      fc_term_puts("error reading sector\n");
      return 1;
    }
    fc_term_puts("\rsector: ");
    fc_term_puts(fc_str_from_uint(sno));
...
int create_disk(struct DeviceServiceRoutine* source_dsr, char* source, struct DeviceServiceRoutine* dest_dsr, char* dest) {
  ...
  unsigned int iocode = fc_path_to_iocode(dest);
  int ferr = fc_lvl2_sector_read(dest_dsr->crubase, iocode, 0, sector_buf);
  if (ferr) {
    fc_term_puts("error reading destination disk\n");
    return 1;
  }
  ...
  if (ferr) {
    fc_term_puts("error opening source file: ");
    fc_term_puts(source);
    fc_term_puts("\n");
    return 1;
  }
  ...
  for(int sno = 0; sno < total_sectors; sno++) {
    ...
    if (ferr) {
      fc_term_puts("error reading record\n");
      return 1;
    }
    ...
    if (ferr) {
      fc_term_puts("error writing sector\n");
      return 1;
    }
    fc_term_puts("\rsector: ");
    fc_term_puts(fc_str_from_uint(sno));
...
int fc_main(char* args) {
  ...
  int read = 0;
  int write = 0;
  if (fc_str_cmp_icase(tmp, "/r") == 0) {
    read = 1;
  } else if (fc_str_cmp_icase(tmp, "/w") == 0) {
    write = 1;
  } else {
    fc_term_puts("Incorrect arguments\n\n");
    return usage();
  }

  // copy source arg into tmp
  cursor = fc_str_token_next(tmp, cursor, ' ');
  char source[256];
  struct DeviceServiceRoutine* source_dsr;
  fc_path_parse(tmp, &source_dsr, source, PR_REQUIRED);
  if (source_dsr == 0) {
    fc_term_puts("error bad source specified\n\n");
    return usage();
  }
  ...
  fc_path_parse(tmp, &dest_dsr, destination, PR_REQUIRED);
  if (dest_dsr == 0) {
    fc_term_puts("error bad destination specified\n\n");
    return usage();
  }
