...
void FC_SAMS(0,init_history()) {
  ...
  map_history();
  fc_str_set((char*)SAMS_ADDR, 0, 0x1000); 
...
void FC_SAMS(0,history_push(char* link)) {
  ...
  char tmp[256];
  fc_str_copy(tmp, link);
  // now bring in the history page
  map_history();

  int len = fc_str_len(tmp);
...
int FC_SAMS(0,history_get(char* dst, int i)) {
  ...
  fc_str_ncopy(dst, entry->data, entry->length);
  return entry->length;
}
...
void FC_SAMS(0,show_history()) {
  ...
  char nl[2] = "\n";
  char line[256];
  fc_str_set(line, 0, 256);
  fc_str_copy(line, "# Virgil99 Browser History\n\n");
  page_from_buf(line, fc_str_len(line));
  int link_idx = 0;
  int len = 1;
  while(len > 0) {
    fc_str_set(line, 0, 256);
