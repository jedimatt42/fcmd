...
int main(char* args) {
  ...
  str_set((char*)&state, 0, sizeof(struct State));
  str_ncopy(state.newurl, args, 256);
...
void on_exit() {
  mouse_hide();
  sams_free_pages(state.page_count + 1 /* 1 for history */);
}
...
void FC_SAMS(0, open_url(char* url)) {
  ...
  if (str_startswith(url, "http:") || str_startswith(url, "https:") || str_startswith(url, "gopher:")) {
    set_error("unsupported protocol", 0);
    return;
  }

  if (!str_cmp(url, "about:")) {
    prepare_for_builtin_url(url);
    about();
    return;
  }

  if (!str_cmp(url, "history:")) {
    prepare_for_builtin_url(url);
    show_history();
    return;
  }

  if (!str_cmp(url, "bookmarks:")) {
    prepare_for_builtin_url(url);
    show_bookmarks();
    return;
  }
  ...
  if(tls_connect(SOCKET_ID, hostname, port)) {
    state.error[0] = 0;
    ...
    int i = str_index_of(line, 13);
    if (i != -1) {
      line[i] = 0;
    }
    i = str_index_of(line, 10);
    if (i != -1) {
      line[i] = 0;
    }
      ...
      case '1':
	{
	  ...
	  int l = str_len(state.lasturl);
	  str_ncopy(state.newurl, state.lasturl, 256);
	  state.newurl[l++] = '?';
	  str_ncopy(state.newurl + l, query, 256 - l);
	  state.cmd = CMD_RELOAD;
	}
	break;
      case '3':
	{
	  str_ncopy(state.newurl, line + 3, 256);
	  state.cmd = CMD_RELOAD;
	}
	break;
      case '4':
...
void send_request(char* request) {
  char tmp[256];
  str_copy(tmp, request);
  str_cat(tmp, CRLF);
  tls_send_chars(SOCKET_ID, tmp, str_len(tmp));
}
...
void handle_success(char* line) {
  ...
  tok = str_token(0, ';');
  if (str_startswith(tok, "text/gemini")) {
    state.cmd = CMD_READPAGE;
  } else if (str_startswith(tok, "text/plain")) {
    state.cmd = CMD_READPAGE;
  } else if (str_startswith(tok, "application/octet-stream")) {
    restore_url();
    ...
    char msg[80];
    str_copy(msg, "uknown mime-type: ");
    str_ncopy(msg + 18, tok, 80-18);
    set_error(msg, 0);
  }
