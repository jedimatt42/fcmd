...
int main(char* args) {
  ...
  sockbuf_init(&control, TCP, 0);
  sockbuf_init(&data, TCP, 1);
  ...
  if (args && args[0]) {
    str_copy(commandbuf, "open ");
    str_cat(commandbuf, args);
    str_token(commandbuf, ' ');
  ...
  while(1) {
    term_puts("ftp> ");
    str_set(commandbuf, 0, 120);
    term_gets(commandbuf, displayWidth - 3, 1);
    term_putc('\n');

    char* tok = str_token(commandbuf, ' ');
    if (!str_cmp_icase("open", tok)) {
      ftpOpen();
      continue;
    } else if (!str_cmp_icase("bye", tok) || !str_cmp_icase("quit", tok) || !str_cmp_icase("exit", tok)) {
      if (connected) {
        ftpQuit();
      }
      return 0;
    } else if (!str_cmp_icase("help", tok)) {
      term_puts(" -- ftp version 1.1 -- \n");
      term_puts("open <hostname> [port] - connect to an ftp server, defaults to port 21\n");
      term_puts("dir [/w] [pathname] - list directory\n");
      term_puts("  alias: ls\n");
      term_puts("pwd - show current server directory\n");
      term_puts("cd <pathname> - change server directory location\n");
      term_puts("get <filename> [tiname] - retrieve a file\n");
      term_puts("lcd <pathname> - change directory on local machine\n");
      term_puts("ldir [pathname] - list local machine directory\n");
      term_puts("bye - close connection\n");
      term_puts("  aliases: exit, quit\n");
      continue;
    } else if (!str_cmp_icase("lcd", tok)) {
      localCmd("CD");
      continue;
    } else if (!str_cmp_icase("ldir", tok)) {
      localCmd("DIR");
      continue;
   } else if (connected) {
      if (!str_cmp_icase("pwd", tok)) {
        ftpPwd();
        continue;
      } else if (!str_cmp_icase("cd", tok)) {
        ftpCd();
        continue;
      } else if (!str_cmp_icase("dir", tok) || !str_cmp_icase("ls", tok)) {
        ftpDir();
        continue;
     } else if (!str_cmp_icase("get", tok)) {
        ftpGet();
        continue;
      }
    }
    term_puts("try 'help' to see list of commands\n");
  }
...
void ftpOpen() {
  char* host = str_token(0, ' ');
  if (!host) {
    term_puts("Error, no host provided.\n");
    return;
  }
  str_copy(hostname, host); // store for pasv connections later.
  char* port = str_token(0, ' ');
  if (!port) {
    port = "21";
  } else {
    int pint = str_to_int(port);
    if (!pint) {
      term_puts("Error, bad port specified.\n");
      return;
    }
  }

  int res = tcp_connect(control.socket_id, host, port);
  if(!res) {
    term_puts("Error, connecting to host\n");
    connected = 0;
    return;
  }
  connected = 1;
  ...
  while(code != 230) {
    ...
    while(!(code == 230 || code == 530)) {
      ...
      str_set(passwd, 0, 20);
      term_puts("password: ");
      term_gets(passwd, 20, 1);
      int plen = str_len(passwd);

      for(int i=0; i<plen; i++) {
        term_putc(8); // backspace
      }
      for(int i=0; i<plen; i++) {
        term_putc('*');
      }
...
void handleTransfer(char* type, data_handler_func dataHandler, char* params[]) {
  ...
  int res = tcp_connect(data.socket_id, hostname, str_from_uint(port));
  if (res) {
    dataHandler(params);
  }
  tcp_close(data.socket_id);
  drainChannel(&control);
}
...
void ftpDir() {
  ...
  if (str_startswith("/w ", tok)) {
    nlist = 1;
    tok = str_token(0, ' '); // consume the "/w"
    tok = str_token_peek(0, 0);
  }
...
void onReceiveFile(char* params[]) {
  ...
  unsigned int iocode = path_to_iocode(currentPath);
  char block[256];

  int len = sockbuf_readstream(&data, block, 128);
  term_puts("read ");
  term_puts(str_from_uint(len));
  term_puts(" bytes of data\n");
  ...
  if (len == 128 && isTiFiles(tifiles)) {
    term_puts("found TIFILES header\n");
    ...
    term_puts("setdir: ");
    term_puts(currentPath);
    term_putc('\n');
    lvl2_setdir(currentDsr->crubase, iocode, (char*)currentPath);

    int ferr = lvl2_output(currentDsr->crubase, iocode, params[1], 0, addInfoPtr);
    if (ferr) {
      term_puts("Error, could not output file\n");
    }
    else {
      int totalsectors = tifiles->sectors;
      int secno = 0;
      while (secno < totalsectors) {
        len = sockbuf_readstream(&data, block, 256); // now work in single block chunks.
        vdpmemcpy(vdp_io_buf, block, 256);
        addInfoPtr->first_sector = secno++;
        ferr = lvl2_output(currentDsr->crubase, iocode, params[1], 1, addInfoPtr);
        if (ferr) {
          term_puts("Error, failed to write block\n");
        }
        else {
          term_putc('.');
        }
    ...
    term_puts("foreign file, will use D/F 128\n");
    if (len == 0) {
      term_puts("Error, no file data received\n");
      tcp_close(data.socket_id);
      return;
    }
    char fullfilename[256];
    str_copy(fullfilename, currentPath);
    str_cat(fullfilename, params[1]);

    struct PAB pab;
    int ferr = dsr_open(currentDsr, &pab, fullfilename, DSR_TYPE_OUTPUT, 128);
    while (len > 0 && !ferr) {
      ferr = dsr_write(currentDsr, &pab, block, 128);
      if (ferr) {
        term_puts("Error, writing file\n");
        return;
      }
      len = sockbuf_readstream(&data, block, 128);
      term_putc('.');
    }
    if (dsr_close(currentDsr, &pab)) {
      term_puts("Error, closing file\n");
      return;
    }
  }
}
...
void ftpGet() {
  char* tok = str_token(0, ' ');
  if (!tok) {
    term_puts("Error, no file specified.\n");
    return;
  }
  char block[256];
  char safetiname[12];
  str_set(safetiname, 0, 12);
  char* tiname = str_token(0, ' ');
  if (!tiname) {
    for (int i = 0; i < 10;i++) {
      if (tok[i] == '.') {
        safetiname[i] = '/';
...
int sendFtpCommand(char* command, char* argstring) {
  char ftpcmd[80];
  str_copy(ftpcmd, command);
  if (argstring != 0) {
    str_cat(ftpcmd, " ");
    str_cat(ftpcmd, argstring);
  }
  str_cat(ftpcmd, EOL);
  int len = str_len(ftpcmd);
  int res = tcp_send_chars(control.socket_id, ftpcmd, len);
  if (!res) {
    term_puts("Error, server disconnected\n");
    return -1;
  }
...
unsigned int sendFtpPasv() {
  ...
  */
  ...
  char* line = sockbuf_readline(&control);
  term_puts(line);
  if (!str_startswith(line, "227")) {
    return 0;
  }
  char* tok = str_token(line, '(');
  for(int i=0; i<4; i++) {
    tok = str_token(0, ',');
  }
  tok = str_token(0, ',');
  unsigned int port = ((unsigned int)str_to_int(tok)) << 8;
  tok = str_token(0, ')');
  port += (unsigned int)str_to_int(tok);
...
int getFtpCode(struct SocketBuffer* socket_buf) {
  ...
  char* line = sockbuf_readline(socket_buf);
  term_puts(line);
  // get code from first response...
  int code = str_to_int(line);
  return code;
}
...
void drainChannel(struct SocketBuffer* socket_buf) {
  ...
  int retries = 0;
  while(retries < 2) {
    int datalen = sockbuf_readstream(socket_buf, buf, 255); // stop short to ensure last character remains nul
    buf[datalen] = 0;
    term_puts(buf);
    if (datalen) {
      retries = 0;
